...
- [ ] Agent 状态可视化（Harness: 验证支柱）
  ...
  - 实时状态监控
  - Token 用量统计
- [ ] **可观测性与评测体系**（Harness: 验证支柱）
  - **Trace 集成**：接入 [Arize Phoenix](https://github.com/Arphoenix-ai/phoenix)（开源自托管优先）或 LangSmith，记录每次 Agent 调用的完整 span（输入/输出/工具调用/延迟/token）
    - LangGraph 原生支持 LangSmith tracer，一行配置启用：`LANGCHAIN_TRACING_V2=true`
    - Phoenix 可本地部署，数据不出境，适合自托管场景
  - **Eval Pipeline**：针对核心场景构建评测集
    - 意图路由准确率（用户请求 → 正确 Skill 路由）
    - 工具调用成功率（tool_call 触发 → 正确参数 → 成功执行）
    - 记忆召回相关性（retrieved memory precision@k）
    - 端到端任务完成率（参考 SWE-bench 模式，自建小型 benchmark）
  - **线上质量监控**：对 production 流量采样，自动检测回归
    - Ragas 评分（faithfulness、answer relevancy、context precision）
    - 用户反馈信号写回（👍/👎 → 自动标注，沉淀到 eval 数据集）
  - **成本追踪**：按 App / 用户 / 模型维度统计 token 消耗和 API 费用
