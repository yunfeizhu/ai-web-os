...
### 阶段六：单 Agent 稳定化 + 可控性 + 可观测性（第 13-16 周）

> 目标：在单 Agent ReAct 循环稳定的基础上，补齐可控性（Human-in-the-loop）、持久化 checkpoint、以及可观测性。多 Agent 并行作为后续自然演进，不作为本阶段范围。
>
...
- [ ] **Human-in-the-loop 执行确认层**（优先级：高）
  - 利用已预留的 LangGraph  接口落地前端确认节点
  - 危险操作（删除文件、发送邮件、批量改写）在 Harness 层拦截后暂停，向前端发送确认事件
  - 用户确认 → ；用户取消 → 终止并说明原因
  - 复杂多步任务可选在执行前展示计划摘要，由用户决定是否继续

- [ ] **持久化 Checkpoint（PostgresSaver）**（优先级：高）
  - 将  升级为 LangGraph 
  - API 进程重启后会话 checkpoint 不丢失，支持长任务恢复
  - 为后续步骤回放与多 Agent 调试提供基础

- [ ] **可观测性：Trace 集成**（优先级：中）
  - 接入 [Arize Phoenix](https://github.com/Arize-ai/phoenix)（开源自托管优先）或 LangSmith
  - 记录每次 agent_loop 的：输入消息、工具调用序列、参数、结果、延迟、token 用量
  - 基础 Trace 事件已就位，只需接入导出器
  - 目标：agent 出问题时能在 Phoenix UI 里一眼看到哪一步出错

- [ ] **Eval Pipeline（持续维护）**（优先级：中）
  - 构建  测试集，持续运行而非一次性交付
  - 核心指标：
    - **工具选择准确率**：LLM 看到 tool description 后是否选对工具（替代已废弃的意图路由准确率概念）
    - **Skill 两阶段成功率**：首次调用返回 guide → 二次调用按正确 query 执行的比例
    - **工具调用成功率**：tool_call → 参数合法 → 成功执行的比例
    - **端到端任务完成率**：用户请求到最终正确回答的比例
  - 按模型/provider 分组对比，找出 function calling 能力差异
...
- [ ] **Lead Agent + Sub-Agent 并行架构**
  - 前提：单 Agent 工具边界、状态流、失败修正已稳定；可观测性已就位
  - Lead Agent 意图理解 + 任务拆解 + Sub-Agent 孵化；Sub-Agent 独立上下文 + 独立工具集 + 并行执行
  - 结果由 Lead Agent 合成；调度树在可视化层可展示

- [ ] **消息总线（Message Bus）**
  - 仅在真正需要多 Agent 并发、后台长任务、跨窗口共享状态时引入
  - 当前单 Agent 阶段过早引入只会增加系统复杂度

- [ ] **本地优先配置与数据归属统一化**
  - 明确本地配置文件 / 浏览器存储 / 服务端缓存 / 系统持久化数据的边界
  - 对敏感配置坚持本地优先，不代持用户密钥

- [ ] **性能与产品收尾**
  - 窗口虚拟化（只渲染可见窗口内容）
  - WebSocket 重连与心跳
  - 前端代码分割（按 App / Skill 拆分 chunk）
  - 主题系统：Light / Dark、自定义主题色、窗口动画
---
