...
- 向 Lead Agent 返回最终摘要、EvidenceBundle、来源、失败原因，以及实验性的 compact `toolEvidence` / `mergedToolResults` 原始工具结果交接
...
- [x] **Manager Subagents 1.0（多 Agent 初版）**
  ...
  - 并行执行最多 4 个子任务，结果以结构化 JSON 回传给 Lead Agent 综合；子 Agent 的自然语言 answer 用于人类可读调试，Lead 优先消费 EvidenceBundle 中的 `facts` / `sources` / `missingFields` / `capabilitiesUsed`
  ...
  - 搜索/抓取工具已抽象为 capability policy：`search.discovery` 结果足够时跳过 `web.extract` / `web.fetch`，避免 Tavily Extract 或未来 SearXNG/Reader 类工具重复抓正文；部分 Extract 成功结果不再整步标红；子 Agent 的充分搜索证据会随 EvidenceBundle 传递给 Lead Agent，搜索标题/摘要/链接/时间会确定性提升为 facts，避免“工具搜到了但最终没输出”
  - 实验性开启 `toolEvidence` / `mergedToolResults`：子 Agent 的 compact 工具结果会合并进入 `delegate_task` 返回，Lead Agent 可直接看到 Skill/API 原始输出，便于验证结构化抽取漏字段问题
...
- [x] **Eval Pipeline（持续维护）**（优先级：中）✅ 已完成
  - `eval_agent_harness.py` 扩展至 21 个测试用例（持续维护）
  - 覆盖 `tool_requires_confirmation`、skill 工具描述长度、confirmation_store 完整流程、子 Agent 工具面隔离、委派 spec 归一化、子 Agent 启动回归、并发工具事件 ID 隔离、当前轮工具结果压缩
  - 新增 capability policy 与 EvidenceBundle 回归：通用 Search/Extract 能力推断、搜索充分时跳过正文抓取、天气 URL 归一化、Extract 部分成功结果校验、子 Agent 结构化事实/来源/缺失字段交接、搜索结果确定性事实抽取、`mergedToolResults` 原始工具结果交接
