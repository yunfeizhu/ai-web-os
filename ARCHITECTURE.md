...
│   │   │   │   │   ├── DesktopIcon.tsx       # 桌面图标（双击打开 Skill）
│   │   │   │   │   ├── Taskbar.tsx           # 底部任务栏
│   │   │   │   │   ├── StartMenu.tsx         # 开始菜单（Skill 列表、搜索）
...
│   │   │   │   └── useSkill.ts               # Skill 生命周期 hook
│   │   │   │
│   │   │   ├── lib/
│   │   │   │   ├── api-client.ts             # HTTP 客户端
│   │   │   │   ├── ws-client.ts              # WebSocket 客户端
│   │   │   │   ├── skill-registry.ts         # 前端 Skill 注册
│   │   │   │   └── utils.ts                  # 工具函数
│   │   │   │
│   │   │   └── types/
│   │   │       ├── window.ts                 # 窗口类型定义
...
interface WindowManagerState {
  ...
  // 操作方法
  openWindow: (skillId: string, props?: Record<string, unknown>) => string;
...
│   ├── StartButton → StartMenu (Skill launcher)
...
function Window({ windowId }: { windowId: string }) {
  ...
  const SkillComponent = useSkillComponent(window.skillId);
    ...
    async (
      ...
      return new Promise<string>((resolve, reject) => {
        ...
        ws.send({
          type: "agent_invoke",
          requestId,
          payload: { skillId, message, sessionId: currentSessionId },
        });
      });
    },
    [ws],
...
CREATE TABLE desktop_layouts (
    ...
    icons JSONB DEFAULT '[]',              -- [{skillId, x, y}]
    taskbar_pins JSONB DEFAULT '[]',       -- [skillId, ...]
...
- [x] StartMenu（Skill 列表、搜索）
- [x] ContextMenu（右键菜单）
- [ ] 窗口 snap（拖到屏幕边缘吸附）
- [x] Zustand stores（windowStore, settingsStore）
- [x] 第一个 Skill UI：Settings（API Key 配置、壁纸、主题）
