...
TOOL_USE_REMINDER = (
    "[系统提示] 如果本次请求需要获取实时数据或执行操作，你 **必须** 通过 function calling 调用对应工具，"
    "**禁止** 根据之前的对话历史仿写或编造工具返回结果。每次查询都可能返回不同数据。"
)
...
def prepare_messages(
    ...
    processed = _summarise_old_tool_results(history)

    # -- Step 2: inject tool-use reminder -------------------------------
