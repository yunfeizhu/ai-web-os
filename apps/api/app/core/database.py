...
SCHEMA_PATCHES: dict[str, list[str]] = {
    ...
    "channel_bindings": [
        "ADD COLUMN IF NOT EXISTS channel VARCHAR(64) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS account_id VARCHAR(128) DEFAULT 'default'",
        "ADD COLUMN IF NOT EXISTS chat_type VARCHAR(32) DEFAULT 'private'",
        "ADD COLUMN IF NOT EXISTS external_chat_id VARCHAR(256) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS external_user_id VARCHAR(256) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS conversation_id VARCHAR(36) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS user_id VARCHAR(128) DEFAULT 'default'",
        "ADD COLUMN IF NOT EXISTS app_id VARCHAR(128) DEFAULT 'ai-chat'",
        "ADD COLUMN IF NOT EXISTS model VARCHAR(128) DEFAULT 'kimi-k2.5'",
        "ADD COLUMN IF NOT EXISTS provider_id VARCHAR(128) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS enabled BOOLEAN DEFAULT TRUE",
        "ADD COLUMN IF NOT EXISTS metadata_json JSON DEFAULT '{}'::json",
        "ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()",
        "ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()",
    ...
    "channel_messages": [
        "ADD COLUMN IF NOT EXISTS channel VARCHAR(64) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS account_id VARCHAR(128) DEFAULT 'default'",
        "ADD COLUMN IF NOT EXISTS direction VARCHAR(32) DEFAULT 'inbound'",
        "ADD COLUMN IF NOT EXISTS external_message_id VARCHAR(256) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS dedupe_key VARCHAR(768) DEFAULT ''",
        "ADD COLUMN IF NOT EXISTS status VARCHAR(32) DEFAULT 'received'",
        "ADD COLUMN IF NOT EXISTS raw_payload JSON DEFAULT '{}'::json",
        "ADD COLUMN IF NOT EXISTS normalized_payload JSON DEFAULT '{}'::json",
