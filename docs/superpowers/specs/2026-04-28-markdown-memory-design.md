...
AI-Native OS 当前记忆实现以 `mem0 + Qdrant` 为主。聊天请求开始时，后端会从 mem0 搜索相关记忆并注入系统提示词；回复结束后，再把本轮用户消息和助手回复异步交给 mem0 抽取并写入长期记忆。
...
| 默认数据根目录 | `AI_NATIVE_OS_HOME`，默认 `~/.ai-native-os` |
| 默认记忆目录 | `~/.ai-native-os/memory` |
...
- AI-Native OS 的扩展和记忆系统应坚持本地优先，关键资产优先落在可编辑的本地 Markdown 文件中。 <!-- memory:id=mem_20260428_c42d18; source=manual; confidence=1.0; updated=2026-04-28T21:31:00+08:00 -->
...
- frontmatter 可选；缺失时按 `schema=ai-native-memory.v1` 兼容解析。
...
```markdown
---
schema: ai-native-daily-memory.v1
date: 2026-04-28
profile: default
---
...
- 用户倾向让 AI-Native OS 的记忆、Skills、扩展都采用本地可编辑文件。 <!-- candidate:id=cand_20260428_1a72b0; conversation=conv_xxx; confidence=0.86; status=pending -->
...
- AI-Native OS 的记忆系统应以本地 Markdown 作为权威源。
...
```json
{
  "schema": "ai-native-memory-state.v1",
  "profile": "default",
...
```json
{
  "schema": "ai-native-memory-candidates.v1",
  "items": [
    {
      "id": "cand_20260428_1a72b0",
      "text": "用户倾向让 AI-Native OS 的记忆、Skills、扩展都采用本地可编辑文件。",
